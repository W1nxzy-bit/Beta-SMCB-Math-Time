...
/* GENERATE QUESTIONS – DIVISION (AVERAGE MODE) */
function generateQuestions(){
    ...
    for(let i = 0; i < TOTAL; i++){
        ...
        // average difficulty but still kid-friendly
        do{
            divisor = Math.floor(Math.random() * 7) + 2; // 2–8
            answer  = Math.floor(Math.random() * 6) + 2; // 2–7
            dividend = divisor * answer;                 // always divisible
        }while(dividend > 30);                            // keep numbers manageable

        let choices = new Set([answer]);
        while(choices.size < 3){
            choices.add(Math.floor(Math.random() * 12) + 1);
        }
