...
/* GENERATE QUESTIONS – PLACE VALUE (AVERAGE MODE, 4 CHOICES) */
function generateQuestions(){
    ...
    for(let i = 0; i < TOTAL; i++){
        ...
        }while(new Set(digits).size !== 4); // no repeated digits

        // pick which digit to ask about
        let index = Math.floor(Math.random() * 4);
        let digit = digits[index];

        // determine correct place value
        let correctAnswer;
        if(index === 0) correctAnswer = "Thousands";
        else if(index === 1) correctAnswer = "Hundreds";
        else if(index === 2) correctAnswer = "Tens";
        else correctAnswer = "Ones";

        // four choices
        let choices = ["Ones", "Tens", "Hundreds", "Thousands"];
