...
/* GENERATE QUESTIONS – PLACE VALUE */
function generateQuestions(){
    ...
    for(let i = 0; i < TOTAL; i++){
        ...
        // generate a 3-digit number with NO repeated digits
        do{
            number = Math.floor(Math.random() * 900) + 100; // 100–999
            digits = number.toString().split("");
        }while(new Set(digits).size !== 3); // ensure all digits are different

        // choose which digit to ask about
        let index = Math.floor(Math.random() * 3);
        let digit = digits[index];

        // determine correct place value
        let correctAnswer;
        if(index === 0) correctAnswer = "Hundreds";
        else if(index === 1) correctAnswer = "Tens";
        else correctAnswer = "Ones";

        // answer choices
        let choices = ["Ones", "Tens", "Hundreds"];
