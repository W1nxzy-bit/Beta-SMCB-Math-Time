...
/* GENERATE QUESTIONS – MULTIPLICATION (DIFFICULT MODE, 4 CHOICES) */
function generateQuestions(){
    ...
    for(let i = 0; i < TOTAL; i++){
        ...
        // harder factors but still Grade 1–3 appropriate
        do{
            a = Math.floor(Math.random() * 8) + 2; // 2–9
            b = Math.floor(Math.random() * 8) + 2; // 2–9
            product = a * b;
        }while(product > 81); // keep within Grade 3 range

        let choices = new Set([product]);
        while(choices.size < 4){
            // close, realistic wrong answers
            choices.add(product + Math.floor(Math.random() * 10) - 5);
        }
