...
/* GENERATE QUESTIONS – SUBTRACTION (DIFFICULT MODE, 4 CHOICES) */
function generateQuestions(){
    ...
    for(let i = 0; i < TOTAL; i++){
        ...
        // harder numbers, borrowing required sometimes
        do{
            a = Math.floor(Math.random() * 50) + 30; // 30–79
            b = Math.floor(Math.random() * 30) + 10; // 10–39
        }while(a <= b); // ensure positive answer

        answer = a - b;

        let choices = new Set([answer]);
        while(choices.size < 4){
            // realistic nearby wrong answers
