...
/* GENERATE QUESTIONS – ADDITION (DIFFICULT MODE, 4 CHOICES) */
function generateQuestions(){
    ...
    for(let i = 0; i < TOTAL; i++){
        ...
        // harder numbers but still Grade 1–3 safe
        do{
            a = Math.floor(Math.random() * 40) + 10; // 10–49
            b = Math.floor(Math.random() * 30) + 10; // 10–39
            sum = a + b;
        }while(sum > 99); // keep within 2 digits

        let choices = new Set([sum]);
        while(choices.size < 4){
            // close distractors (realistic wrong answers)
            choices.add(sum + Math.floor(Math.random() * 10) - 5);
        }
